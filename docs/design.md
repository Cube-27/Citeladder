...
**AI Visibility.** Exactly three tabs: Trends (default), Mentions & Citations, Query Fanout; no parallel Overview or page-local project switcher. Trends presents selection/coverage context, three headline measures (Visibility, Share of voice, Owned citation rate), one metric-selectable history chart, one competitor comparison table, and prompt/model outcomes. Each measure retains its denominator and compatible change; matched subsets disclose their own values and coverage. Use timestamp spacing and break chart lines at incompatible or unavailable measurements. A single point uses a compact textual state. Hide uncomputed sentiment/position columns. Sources and Answers are modes inside Mentions & Citations. Evidence uses ruled execution rows, original-answer links and collapsed Provenance disclosures. Retain shared metric, table, typography, spacing, responsive and control primitives throughout; secondary mobile fields use labelled details without removing actions.
