...
| Commerce Suite (products/attribution) | [`../plans/v4-commerce-suite-m2-m5.md`](../plans/v4-commerce-suite-m2-m5.md) | **In progress** (branch `vorflux/commerce-suite`, plan + status: [`../plans/commerce-suite/`](../plans/commerce-suite/)). Shipped slices: M2a analyzer v2 + shopping-surface slot (`app/analysis/product_scoring.py`, `app/domain/products/visibility.py`), GA4 A1 attribution (`app/domain/attribution/`, `app/api/commerce.py`), Shopify GraphQL OAuth + durable cursor sync + catalog/feed/order derivation (`app/connectors/integrations/shopify.py`, `app/domain/commerce/`), and the three-tab Commerce workspace frontend (`frontend/components/products/`, `frontend/lib/api/{commerce,attribution}.ts`). Remaining: A2 order-referrer links + combined snapshots + order drill-down (M5 Layer A2), catalog-health + attribution-recompute routes, and two M2a component test files — see the handoff doc for the exact leftover list. |
