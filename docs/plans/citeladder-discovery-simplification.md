...
5. **Improve subsequent prompt grounding.** Keep Generate prompts' requested-count,
   topic, cohort, capacity and locking semantics. Feed canonical context through
   its existing orchestration. Improve persisted GSC evidence selection using
   relevant project vocabulary, uncovered needs, then impressions/clicks with
   deterministic ties. Prefer the latest complete observation window, targeting
   approximately 28 days where available, and supply at most 30 representative
...
separate. Automatic grounding does not implement or complete it.
...
- GSC relevance, observation-window ordering, representative selection,
  provenance and absence of GSC.
