...
/** One `field: value` SSE frame, before JSON/schema validation. */
...
/**
 * Split a buffer into complete SSE frames, returning the leftover partial.
 *
 * Frames are separated by a blank line. A chunked response can split a frame
 * anywhere, so the remainder is carried into the next read rather than parsed
...
/**
 * Parse one raw frame's fields. Keep-alive comment lines (`:` prefix) and
 * unknown fields are ignored; `data:` lines concatenate per the SSE spec.
 */
export function parseFrame(frame: string): RawSseFrame {
