...
const guidance = {
  id: AUDIT, opportunity_id: OPP, input_hash: 'hash', findings: ['Brand is absent'], recommendations: ['Publish a comparison page'],
  source_analysis_ids: [AUDIT], source_issue_ids: [], source_metric_ids: [AUDIT], analyzer_version: 'a1', rule_version: 'r1', formula_version: 'f1',
  generator_version: 'g1', prompt_version: 'p1', provider: 'deterministic', model: 'template', created_at: '2026-07-24T00:00:00Z',
};

describe('opportunity guidance contract', () => {
  it('creates immutable guidance with an idempotency key', async () => {
    mswServer.use(http.post(`/api/v1/opportunities/${OPP}/guidance`, ({ request }) => {
    ...
    expect(strictValidate(opportunityGuidanceItemSchema, guidance, 'guidance').recommendations).toHaveLength(1);
  });
