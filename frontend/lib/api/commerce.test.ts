...
const candidate = {
  id: UUID, run_id: UUID2, task_id: UUID3, artifact_id: '33333333-3333-4333-8333-333333333332',
  candidate_kind: 'own', competitor_id: null, identity: { name: 'Trail shoe' },
  extraction_confidence: 0.9, created_at: '2026-07-24T06:00:00Z', matches: [],
};
...
describe('commerceApi', () => {
  ...
  it('uses same-origin discovery and comparison endpoints with strict DTOs', async () => {
    const fetchMock = vi.fn()
      .mockResolvedValueOnce(jsonResponse({ accepted: [{ name: 'Trail shoe' }], duplicates: [], errors: [], truncated: false }))
      .mockResolvedValueOnce(jsonResponse({ id: UUID2, project_id: UUID2, input_kind: 'upload', status: 'queued', configuration: {}, discovery_version: 'v1', created_at: '2026-07-24T06:00:00Z', completed_at: null, candidates: [candidate] }))
      .mockResolvedValueOnce(jsonResponse([candidate]))
      .mockResolvedValueOnce(jsonResponse({ id: UUID3, project_id: UUID2, competitor_id: null, source_catalog_ids: { products: [UUID], competitor_products: [] }, source_artifact_ids: [], matcher_version: 'v1', comparison_version: 'v1', comparison: { coverage: {}, items: [] }, truncated: false, created_at: '2026-07-24T06:00:00Z' }));
    vi.stubGlobal('fetch', fetchMock);
    const { commerceApi } = await import('./commerce');
    await commerceApi.previewDiscovery(UUID2, { csv_text: 'name\nTrail shoe' });
    await commerceApi.createDiscoveryRun(UUID2, { input_kind: 'upload', rows: [{ name: 'Trail shoe' }] });
    await commerceApi.listDiscoveryCandidates(UUID2, UUID2);
    await commerceApi.createComparison(UUID2);
    expect(String(fetchMock.mock.calls[0]?.[0])).toBe(`/api/v1/projects/${UUID2}/commerce/discovery/preview`);
    expect(String(fetchMock.mock.calls[2]?.[0])).toBe(`/api/v1/projects/${UUID2}/commerce/discovery/candidates?run_id=${UUID2}`);
    expect(String(fetchMock.mock.calls[3]?.[0])).toBe(`/api/v1/projects/${UUID2}/commerce/comparisons`);
  });
});
