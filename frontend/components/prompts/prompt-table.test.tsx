...
describe('PromptTable pagination', () => {
  it('pages through rows with the mono indicator and ghost buttons', async () => {
    ...
    expect(screen.getByText((_, el) =>
        el?.tagName === 'SPAN' && el.textContent?.replace(/\s+/g, ' ').trim() === '1–10 of 12 prompts',
      )).toBeInTheDocument();
    ...
    expect(screen.getByText((_, el) =>
        el?.tagName === 'SPAN' && el.textContent?.replace(/\s+/g, ' ').trim() === '11–12 of 12 prompts',
      )).toBeInTheDocument();
  ...
  it('clamps the current page when the list shrinks instead of resetting', async () => {
    ...
    expect(screen.getByText((_, el) =>
        el?.tagName === 'SPAN' && el.textContent?.replace(/\s+/g, ' ').trim() === '1–5 of 5 prompts',
      )).toBeInTheDocument();
    expect(screen.getByRole('button', { name: 'Next page' })).toBeDisabled();
  });
