...
/**
 * Exhaustive over LandingFeature['key'] on purpose: the visuals are bespoke per
 * feature, so a new key must fail to compile here rather than quietly inherit
 * the trend chart — a wrong-but-plausible figure is worse than a missing one.
 */
function FeatureVisual({ kind }: Readonly<{ kind: LandingFeature['key'] }>) {
  switch (kind) {
    ...
    case 'health':
      return <div className="feature-health" aria-hidden="true"><span><Check /> Technical <b>84</b></span><span><Check /> AEO readiness <b>76</b></span></div>;
    case 'benchmark':
      return (
    ...
    default: {
      ...
      throw new Error(`FeatureVisual: unhandled feature key ${String(unhandled)}`);
    }
  }
}
