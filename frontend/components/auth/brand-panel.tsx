...
import { Panel, WallpaperPanel } from '@/components/marketing/scenes/wallpaper-panel';
...
export function AuthBrandPanel() {
  return (
    <WallpaperPanel className="relative col-span-5 flex flex-col rounded-none border-0 border-r px-10 py-8 max-[900px]:hidden">
            ...
            <ul className="border-mkt-line mt-8 grid list-none gap-3 border-t p-0 pt-6">
              {PROOF_POINTS.map((proof) => (
                <li key={proof.lead} className="flex items-center gap-3">
                  ...
                  <span className="text-mkt-sm text-mkt-ink-soft">{proof.lead}</span>
                </li>
              ))}
            </ul>
          </Panel>
