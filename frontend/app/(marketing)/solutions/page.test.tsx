...
describe('Solutions page (public marketing `/solutions`)', () => {
  ...
  it('renders each segment with its lead and a demo CTA', () => {
    render(<Page />);

    const agencies = screen.getByRole('region', { name: 'Agencies' });
    expect(within(agencies).getByText(/per-client workspaces/i)).toBeInTheDocument();
    ...
    expect(within(inHouse).getByText(/cross-engine trend lines/i)).toBeInTheDocument();
    ...
    expect(within(founders).getByText(/seeded sample crawl/i)).toBeInTheDocument();
    ...
    expect(within(commerce).getByText(/rival SKUs displace yours/i)).toBeInTheDocument();
    ...
    expect(within(pr).getByText(/citation ownership per prompt/i)).toBeInTheDocument();
