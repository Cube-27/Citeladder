...
def test_message_layout_is_system_instruction_reference() -> None:
    messages, digest, snapshot = build_messages(
        user_instruction="Write a landing page", context=_context()
    )
...
def test_crawl_injection_stays_in_untrusted_reference_message() -> None:
    ...
    messages, _, _ = build_messages(
        user_instruction="Write a product page", context=_context(_INJECTION)
    )
...
def test_no_citation_machinery_reaches_the_model() -> None:
    ...
    messages, _, _ = build_messages(user_instruction="Write a page", context=_context())
    assert "[[source:" not in "".join(message["content"] for message in messages)


def test_skill_body_precedes_the_user_prompt() -> None:
    messages, _, _ = build_messages(
        user_instruction="Write about school uniforms",
        context=_context(),
        skill_id="faq",
    )
    instruction = messages[1]["content"]
    assert instruction.endswith("USER REQUEST:\nWrite about school uniforms")
    assert instruction.startswith("## Purpose")
    assert "Use when the content is naturally expressed" in instruction


def test_origin_evidence_stays_out_of_the_instruction_message() -> None:
    context = ContentContext(
        issue_block="OPPORTUNITY EVIDENCE\nIssue: Competitors out-cited on sizing",
    )
    messages, _, _ = build_messages(user_instruction="Fix this", context=context)
    assert messages[1]["content"].endswith("Fix this")
    assert "Competitors out-cited on sizing" not in messages[1]["content"]
    assert "Competitors out-cited on sizing" in messages[2]["content"]


def test_reference_message_is_omitted_when_there_is_no_context() -> None:
    messages, _, _ = build_messages(
        user_instruction="Write a page", context=ContentContext()
    )
...
def test_digest_is_stable_and_input_sensitive() -> None:
    context = _context()
    _, digest_a, _ = build_messages(user_instruction="P", context=context)
    _, digest_b, _ = build_messages(user_instruction="P", context=context)
    _, digest_c, _ = build_messages(user_instruction="P2", context=context)
...
def test_snapshot_caps_each_message() -> None:
    messages, _, snapshot = build_messages(
        user_instruction="y" * 10_000, context=_context("x" * 10_000)
    )
