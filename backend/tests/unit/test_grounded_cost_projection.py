...
def _artifact(usage: dict) -> RawResponseArtifact:
    return RawResponseArtifact(
        ...
        transport_model=ROUTE_CHATGPT_PULSE.transport_model,
        answer_text="answer",
        usage=usage,
    )
...
def test_catalog_contains_only_six_exact_measurement_routes() -> None:
    ...
    assert ROUTE_CHATGPT_PULSE.transport_model == "gpt-5.4-nano-2026-03-17"
    assert ROUTE_CHATGPT_BENCHMARK.transport_model == "gpt-5.6-sol"
    assert ROUTE_CLAUDE_PULSE.transport_model == "claude-haiku-4-5-20251001"
    assert ROUTE_CLAUDE_BENCHMARK.transport_model == "claude-sonnet-5"
    assert ROUTE_GEMINI_PULSE.transport_model == "gemini-3.5-flash-lite"
    assert ROUTE_GEMINI_BENCHMARK.transport_model == "gemini-3.6-flash"


def test_unknown_official_price_lines_remain_null() -> None:
    pricing = route_pricing_for(ROUTE_CHATGPT_BENCHMARK, PRICING_CATALOG_VERSION)
...
def test_projection_accepts_only_canonical_usage_keys() -> None:
    pricing = route_pricing_for(ROUTE_CHATGPT_PULSE, PRICING_CATALOG_VERSION)
    assert pricing is not None
