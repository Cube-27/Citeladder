...
Structure: one clear H1; an opening that answers the subject or states the value immediately; H2/H3 sections for distinct reader questions, capabilities or decisions; lists or tables where they communicate better than prose; proof, examples, process or differentiators when supplied; a natural next step when the page has a conversion goal.

Preserve verified pricing, eligibility, geography, company identity, CTA destinations and useful navigation. "Log in", "Sign up" and "Learn more" can be correct labels for those actions; do not replace functional labels with misleading benefit copy. A homepage refresh is not a visual redesign. Do not turn a landing page into a long article or add FAQ as SEO furniture.
...
Structure: the exact product name as H1; a concise definition or value statement; key purchase facts; benefits tied to actual supplied attributes; specifications (a compact table when it helps scanning); use cases; compatibility, sizing, material, care, delivery, returns or other purchase information when supplied; a review section only when real review data exists; a clear purchase next step.
...
Structure: H1 and a concise category definition; who the category is for; the major groups or options; how to choose among them; important attributes, use cases or distinctions; supporting questions only when evidence shows real recurring selection questions; the navigation or purchase next step.
...
Structure: one H1 identifying the organization; a direct definition of the organization and its offering; audience or use cases; history only where durable context supplies it; products, services or expertise; approach or real differentiators; people, locations, credentials or milestones when supplied; a natural next step.
...
Structure: a specific H1; a direct opening answer or thesis; optional key takeaways when they genuinely help; H2 sections answering the reader's major follow-up questions; concrete examples, criteria, steps, evidence or scenarios; a practical decision framework or next steps when appropriate; a closing takeaway that adds something rather than repeating the introduction.

Build headings around real questions, not keyword variants. Define terms at first use. Include a worked example only if genuine or clearly labelled illustrative. A tutorial must contain enough detail to complete the task and acknowledge constraints and failure cases. Use tables only for real comparison or reference value, supported by sourced comparable facts. Link to the commercial or support destination only where the reader needs it.
...
Use an approved original observation, sourced analysis, case example or decision framework, and establish whose voice the post represents without inventing personal experience. Open with the idea, result or tension before the "see more" cut, develop one idea in two to four short paragraphs with specifics and honest limits, and end with a takeaway the reader can act on. Plain-text formatting; no one-word-per-line staircases, fake-humble framing, "Agree?" endings or fabricated engagement.

Deliver: the complete post, supporting-source notes and the target buyer task. Posting is the user's action.

## x — X post or thread

For a thread, the first post carries the strongest useful claim, each following post one supporting point that stands on its own, and the last a concise takeaway. For a single post, return only the strongest self-contained version. Keep every post within the platform limit; no emoji-bullet clutter or sentences split to manufacture a thread.
