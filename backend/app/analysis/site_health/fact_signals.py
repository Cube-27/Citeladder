...
_PROVIDER_RE: Final = re.compile(
    r"^(?P<provider>[A-Z][A-Za-z0-9&'.-]*"
    r"(?:\s+[A-Z][A-Za-z0-9&'.-]*){0,5})\s+"
    r"(?P<verb>provides?|offers?|delivers?|builds?|manages?|measures?|turns?|brings\s+together|"
    r"helps?|enables?|specializes\s+in)\s+",
)
_PROVIDER_NAME_RE: Final = re.compile(
...
def _editorial_lead_candidate(
    ...
    *,
    ...
    if tag != "p" or not seen_identity:
        return ""
    if not _page_owned_node(node, region, container_ids) or is_metadata_or_cta(node):
        return ""
...
def _substantive_proposition(region: Any, container_ids: set[int]) -> str:
    ...
    for paragraph in paragraphs[: taxonomy.REGION_MAX_CONTAINERS_SCANNED]:
        ...
        if len(candidate.split()) >= ANSWER_FIRST_MIN_WORDS and not is_metadata_or_cta(
            paragraph
        ):
...
def _entity_proposition(
    ...
    *,
    ...
    provider = _provider_identity(proposition) or _address_identity(
        region, container_ids
    )
    identity = provider
    capability = _named_capability(proposition)
...
def _provider_identity(proposition: str) -> str:
...
def _address_identity(region: Any, container_ids: set[int]) -> str:
    try:
        addresses = region.xpath(".//address")
    except DOM_ERRORS as exc:
        dom_failure("_address_identity", exc)
        return ""
    for address in addresses[: taxonomy.REGION_MAX_CONTAINERS_SCANNED]:
        if not _page_owned_node(address, region, container_ids):
            continue
        try:
            candidates = (" ".join(str(part).split()) for part in address.itertext())
            name = next((part for part in candidates if part), "")
        except DOM_ERRORS as exc:
            dom_failure("_address_identity", exc)
            continue
...
def _named_capability(proposition: str) -> str:
...
def _next_action_candidate(node: Any) -> str:
    text = " ".join(_text(node).split())
    if not (NEXT_ACTION_RE.search(text) or _is_cta_anchor(node)):
        return ""
