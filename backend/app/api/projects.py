...
from app.domain.source_pages.projection import get_source_page
...
def _source_page_detail(view) -> SourcePageDetail:
...
@router.get(
    "/{project_id}/source-pages/{url_hash}",
    response_model=SourcePageDetail,
)
async def get_source_page_endpoint(
    project_id: uuid.UUID,
    url_hash: Annotated[str, Path(min_length=64, max_length=64)],
    ctx: _WorkspaceDep,
    session: _SessionDep,
) -> SourcePageDetail:
    ...
    return _source_page_detail(view)
...
async def inspect_source_page_endpoint(
    ...
    spend accounting. A page someone asked for is not free, and saying so is
    what keeps the remaining allowance meaningful.
    """
    await _get_project_or_404(session, ctx.workspace_id, project_id)
    view = await get_source_page(
        session,
