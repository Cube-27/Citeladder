...
def _validated_prompt_suggestions(
    ...
    *,
    ...
    for prompt in synthesis.prompts:
        ...
        if len(accepted) >= brand_discovery_settings.synthesis_prompt_count:
            break
    return accepted
...
async def _collect_owned_site(row: BrandDiscovery, data: dict) -> _OwnedSiteEvidence:
    ...
    return _OwnedSiteEvidence(
        ...
        gaps=[] if captured_text.strip() else ["official_site_evidence"],
    )
...
async def _verify_competitor_candidate(
    ...
    *,
    ...
    if domain is None:
        return None
    await _approve_vendor_url(candidate.url)
    verification, capture_method = await _read_competitor_site(candidate)
...
async def _try_verify_competitor_candidate(
    ...
    *,
    ...
    except (FirecrawlUnavailableError, UrlPolicyError, ValueError) as exc:
        logger.info(
...
async def create_project_from_discovery(
    ...
    *,
    ...
    topics = list(row.topics) or [str(profile.get("industry") or "General")]
    topic_rows = [
        Topic(
            id=uuid.uuid4(),
    ...
    prompt_rows = [
        Prompt(
            ...
            topic_id=(
                topics_by_name[suggestion["theme"].casefold()].id
                if suggestion.get("theme", "").casefold() in topics_by_name
                else None
            ),
